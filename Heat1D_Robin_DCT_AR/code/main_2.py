...
def run(config, args):
    ...
    print(f"--- set model {model_config['model_class']}.dtype is {dtype};Total trainable parameters: {total_params}")
    ...
    with torch.no_grad():
        for xx, yy, grid, _ in test_loader:
            ...
            for t in range(init_t, t_train):
                current_time = gridt[t, 0].item()
                inp = xx.reshape(inp_shape)
                t_grid = torch.full((xx.size(0), xx.size(1), 1), current_time, device=device, dtype=dtype,
                                    requires_grad=True)
                out = model(torch.cat([inp, grid, t_grid], dim=-1)).reshape(outp_shape)
                pred[..., t:t + 1, :] = out
    ...
    for e in ebar:
        ...
        for b in trange(len(train_loader), file=desc, desc="batch"):
            ...
            for t in range(init_t, current_t_train):
                ...
                _batch = out.size(0)
                # 下采样评估
            pred_model = torch.cat(pred_list, dim=-2)  # [batch, nx, nt-init_t, c]
            u_t_all = torch.cat(u_t_list, dim=-1)

            pred_full = torch.cat([pred_init, pred_model], dim=-2)
            assert pred_full.shape == yy.shape
            out_data = pred_full[:, ::rx, ::rt, :].reshape(_batch, -1)
            y_data = yy[:, ::rx, ::rt, :].reshape(_batch, -1)
            loss_data = myloss(out_data, y_data)
            if First_data:
                print('out_data.shape, y_data.shape:', pred_full[:, ::rx, ::rt, :].shape, yy[:, ::rx, ::rt, :].shape)
                First_data = False
            # ===== 统一计算loss =====
            loss_f = torch.tensor(0.0, device=device)
            if config['train']['loss_mode'] != 'data':
                # print('u_t_all.shape:', u_t_all.shape)
                residual = compute_heat_residualII(pred_full, D2, u_t_all, init_t)
            ...
            # 更新进度条
            new_desc = (f"Epoch {e + 1}: {desc.read(b)}, t_train: {current_t_train}, "
        ...
        elif config['train']['scheduler'] not in ['cosine_schedule_with_warmup', 'ReduceLROnPlateau']:
            scheduler.step()
