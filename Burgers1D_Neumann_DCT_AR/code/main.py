...
from Burger.datasets import h5DatasetFor1DBurgersII, h5DatasetFor1DBurgers_muti, ResolutionWeightedBatchSampler
...
def run(config):
    ...
    v = 0.01
    if data_config['muti_resolution']:
        ...
        group_weights = {"129": 2, "257": 1, "517": 3}  # 129组的样本扩展为2倍
    ...
    for e in ebar:
        ...
        for b in trange(len(train_loader), file=desc, desc="batch"):
            ...
            """
            loss = 0.0
            xx, yy, grid, resolution = next(train_iter)
            # print(resolution)
            xx, yy, grid = xx.to(device, non_blocking=True), yy.to(device, non_blocking=True), grid.to(device,
                                                                                                       non_blocking=True)  # 确保数据在相同设备上
...
def test(config, args):
    ...
    if args.pretrain is not None:
        ...
        if epoch is None:
            ...
            epoch = 0
        print(f"{datetime.now()} --- model【{args.pretrain}】has been loaded, {epoch} epochs has trained,now lr is {lr_list[-1][0]}")
    else:
        print(os.getcwd())
        ...
        print(f"{datetime.now()} --- current best model has been loaded, {epoch} epochs has trained, now lr is {lr_list[-1][0]}")
