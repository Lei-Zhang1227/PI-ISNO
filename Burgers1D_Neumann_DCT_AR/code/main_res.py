...
def run(config):
    ...
    if data_config['loader'] == 'FixedRes':
        train_data = h5DatasetFor1DBurgersII(case_path(data_config['datapath']),
                                             sub_x=data_config['sub_x'],
                                             sub_t=data_config['sub_t'],
                                             initial_step=data_config['initial_step'])
        train_loader = torch.utils.data.DataLoader(train_data, batch_size=batch_size, shuffle=True,
                                                   num_workers=2, pin_memory=True)
        test_data = h5DatasetFor1DBurgersII(case_path(data_config['datapath']),
                                            sub_x=data_config['sub_x'],
                                            sub_t=data_config['sub_t'],
    ...
    log_dic = os.path.join(first_dic, "logs")
    ...
    for e in ebar:
        ...
        for b in trange(len(train_loader), file=desc, desc="batch"):
            ...
            # print(resolution)
            xx, yy, grid, label = xx.to(device, non_blocking=True), yy.to(device, non_blocking=True), grid.to(device,
                                                                                                              non_blocking=True), label.to(
                device, non_blocking=True)  # 确保数据在相同设备上
            #             print('yy.shape:', yy.shape)
            #             print('xx.shape:', xx.shape)
            if config['train']['loss_mode'] == 'data':
                original_size = xx.shape[1]
                target_size = int(config['train']['data_dim'])
                rx = (original_size - 1) // (target_size - 1)
                xx = xx[:, ::rx, :, :]
                yy = yy[:, ::rx, :, :]
                grid = grid[:, ::rx, ...]
                if First_data:
                    print('rx,xx.shape,yy.shape:', rx, xx.shape, yy.shape)
            optimizer.zero_grad()
            #             print('xx[:, :, 0, :]:', xx[:, :, 0, :])
            #             print('yy[:, :, 0, :]:', yy[:, :, 0, :])
            ...
            for t in range(init_t, current_t_train):
                ...
                # loss += myloss(out.reshape(_batch, -1), wram_x.reshape(_batch, -1))
                pred[..., t:t + 1, :] = out
                xx = torch.cat((xx[..., 1:, :], out), dim=-2)
            assert pred.shape == yy.shape, f"Tensor shapes do not match: {pred.shape} != {yy.shape}"
            if config['train']['loss_mode'] == 'both':
                original_size = pred.shape[1]
                target_size = int(config['train']['data_dim'])
                rx = (original_size - 1) // (target_size - 1)
                out_data = pred[:, ::rx, ::int(config['data']['data_sub_t']), :]
                y_data = yy[:, ::rx, ::int(config['data']['data_sub_t']), :]
                if First_data:
                    print('out_data.shape,y_data.shape:', out_data.shape, y_data.shape)
                    First_data = False
                # loss_data = F.mse_loss(out_data, y_data).to(torch.float32)
                mask = label.squeeze() == 1
                # print('mask:', mask)
                if mask.any():
                    # print('out_data[mask].shape:', out_data[mask].shape)
                    loss_data = F.mse_loss(out_data[mask], y_data[mask]).to(torch.float32)
                else:
                    print('no label sample')
                    loss_data = torch.tensor(0.0, device=out_data.device, dtype=torch.float32)
                loss_fn = PINO_loss_1D if config['train']['residual_mode'] == 'Spectral' else PINO_loss_1DII
                loss_init, loss_f, loss_b = loss_fn(pred, init_x.permute(0, 2, 1), init_t, x_length, time_lentgh)
                total_loss = (loss_init * ic_weight + loss_f * f_weight + loss_data * data_weight).to(torch.float32)
                total_loss.backward()
                grad_norm = clipper.step(model)  # 计算梯度范数
                grad.append([e, loss.item(), grad_norm])
            elif config['train']['loss_mode'] == 'data':
                loss_init, loss_f, loss_b = torch.tensor(0.0, device=device), torch.tensor(0.0,
                                                                                           device=device), torch.tensor(
                    0.0, device=device)
                # original_size = pred.shape[1]
                # target_size = int(config['train']['data_dim'])
                # rx = (original_size - 1) // (target_size - 1)
                out_data = pred[:, ::int(config['data']['data_sub_x']), ::int(config['data']['data_sub_t']), :]
                y_data = yy[:, ::int(config['data']['data_sub_x']), ::int(config['data']['data_sub_t']), :]
                if First_data:
                    print('out_data.shape,y_data.shape:', out_data.shape, y_data.shape)
                    First_data = False
                loss_data = F.mse_loss(out_data, y_data).to(torch.float32)
                ...
                y_data = yy[:, ::rx, ::rt, :]
                loss_data = F.mse_loss(out_data, y_data).to(torch.float32)
                total_loss = loss_f.to(torch.float32)
                total_loss.backward()
                grad_norm = clipper.step(model)  # 计算梯度范数
                grad.append([e, loss.item(), grad_norm])
            ...
            loss_list.append([loss.item(), loss_f.item(), total_loss.item(), e])
            new_desc = f"Epoch {e + 1}: {desc.read(b)},resolution：{resolution.unique()}, t_train: {current_t_train},Loss_data: {loss_data.item():.4e},Loss_BC: {loss_b.item():.4e},Loss_phy: {loss_f.item():.4e},lr:{current_lr}, Grad Norm Threshold:{grad_norm:.2f}"
        ...
        writer.add_scalar("Loss_phy/Epoch", loss_f.item(), e)
        writer.add_scalar("total_loss/Epoch", total_loss.item(), e)
        lr_list.append([current_lr, current_t_train, e])
        # scheduler.step()
        if best_error > batch_loss:
            best_error = batch_loss
            model_save_record.append([e, batch_loss])
...
def test(config, args):
    ...
    test_data_1x = h5DatasetFor1DBurgersII(case_path(data_config['datapath']),
                                           sub_x=1,
                                           sub_t=data_config['sub_t'],
                                           initial_step=data_config['initial_step'], if_test=True)
    ...
    test_data_1_2x = h5DatasetFor1DBurgersII(case_path(data_config['datapath']),
                                             sub_x=2,
                                             sub_t=data_config['sub_t'],
                                             initial_step=data_config['initial_step'], if_test=True)
    ...
    test_data_1_4x = h5DatasetFor1DBurgersII(case_path(data_config['datapath']),
                                             sub_x=4,
                                             sub_t=data_config['sub_t'],
                                             initial_step=data_config['initial_step'], if_test=True)
    ...
    test_data_1_8x = h5DatasetFor1DBurgersII(case_path(data_config['datapath']),
                                             sub_x=8,
                                             sub_t=data_config['sub_t'],
                                             initial_step=data_config['initial_step'], if_test=True)
    ...
    test_data_1_16x = h5DatasetFor1DBurgersII(case_path(data_config['datapath']),
                                              sub_x=16,
                                              sub_t=data_config['sub_t'],
                                              initial_step=data_config['initial_step'], if_test=True)
...
def test2(config, args):
    ...
    with torch.no_grad():
        ...
        for b in tqdm(range(len(test_loader))):
            ...
            mean_residual_pred = torch.mean(torch.abs(Du_pred)).item()
            mean_residual_yy = torch.mean(torch.abs(Du_yy)).item()
            results.append([int(resolution[0]), l2_error, mean_residual_pred, mean_residual_yy])
    # 转换为DataFrame
    ...
    with open(f'{first_dic}/Experiment_record.txt', 'a', encoding='utf-8') as f:
        ...
        headers = ["resolution", "l2_error", "mean_residual_pred", "mean_residual_yy"]
        f.write("\t".join(headers) + "\n")

        # 逐行写入数据
        for _, row in grouped.iterrows():
            line = "\t".join([f"{x:.4f}" if isinstance(x, float) else str(x)
                              for x in row])
            f.write(line + "\n")
        f.close()
    # 计算均值和标准差
