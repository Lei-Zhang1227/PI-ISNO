...
def run(config,args):
    ...
    if data_config['loader'] == 'FixedRes':
        ...
        for idx, name in enumerate(train_data.data_list):
            # 在报错行之前加
#             print(f"name: {name}")
#             print(f"resolution_df sample_ids: {resolution_df['sample_id'].head(10).tolist()}")
#             print(f"name in resolution_df: {name in resolution_df['sample_id'].values}")
    ...
    for e in ebar:
        ...
        for b in trange(len(train_loader), file=desc, desc="batch"):
            ...
            xx, yy, grid, label = xx.to(device, non_blocking=True), yy.to(device, non_blocking=True), grid.to(device,
                                                                                                              non_blocking=True), label  # 确保数据在相同设备上
...
#                     print('rx,xx.shape,yy.shape:', rx, xx.shape, yy.shape)
            ...
            loss_list.append([loss.item(),loss_data.item(), loss_f.item(), total_loss.item(), e])
            new_desc = f"Epoch {e + 1}: {desc.read(b)},resolution：{resolution[0]}, t_train: {current_t_train},Loss_data: {loss_data.item():.4e},Loss_train: {total_loss.item():.4e},Loss_phy: {loss_f.item():.4e},lr:{current_lr}, Grad Norm Threshold:{grad_norm:.2f}"
