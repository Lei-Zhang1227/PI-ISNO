...
def Burgers_residual_i(u, v, D=1):
    '''
...
def PINO_loss(u, u0, v):
    ...
    boundary_u = u[:, index_t, index_x]  # 为啥费这么大劲写成这样，直接[:,0,:]不成吗
    loss_u = F.mse_loss(boundary_u, u0)  # 边界条件为啥计算的是和初始条件的差异？

    Du = Burgers_residual_i(u, v)[:, :, :]
...
def train_burger(model,
    ...
    for e in pbar:
        ...
        for x, y in train_loader:
            ...
            loss_u, loss_f = PINO_loss(out, x[:, 0, :, 0], v)  # 这个意思是只取每个sample的初始条件
