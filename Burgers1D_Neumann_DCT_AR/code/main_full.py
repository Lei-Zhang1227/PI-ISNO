...
def run(config, args):
    ...
    for e in ebar:
        ...
        for xx, yy, _, _ in train_loader:
            ...
            optimizer.zero_grad()
            pred = model(xx)
            # xx.shape:torch.Size([20, 257, 2]),pred.shape:torch.Size([20, 100, 257]),YY.shape:torch.Size([20, 257, 100])
            pred = pred.permute(0, 2, 1)
            assert pred.shape == yy.shape, f"Tensor shapes do not match: {pred.shape} != {yy.shape}"
            u0 = xx[:, :, 0:1]
            ...
            if config['train']['loss_mode'] != 'data':
                loss_fn = PINO_loss_1DIII if config['train']['residual_mode'] == 'Spectral' else PINO_loss_1DII
                _, loss_f, loss_b = loss_fn(full_pred)  # [batch, nx, 1 + n_output_steps]
        ...
        # Epoch结束
        epoch_physics_loss = epoch_physics_loss / len(train_loader)
        avg_epoch_loss_data = epoch_loss_data / len(train_loader)
        epoch_train_loss = epoch_train_loss / len(train_loader)
        avg_grad_norm = np.mean(epoch_grad_norms)
        grad.append([e, avg_epoch_loss_data, avg_grad_norm])
        lr_list.append([current_lr, 100, e])

        # 更新 epoch 进度条描述
        ebar.set_description(
            f"Epoch {e + 1}: Loss_all:{epoch_train_loss:.4e},Loss_data: {avg_epoch_loss_data:.4e},Loss_phy:{epoch_physics_loss:.4e}, Test L2: {test_l2:.4e}, lr: {current_lr:.2e}"
        )
        # ============ Epoch 结束后更新 lr ============
        if scheduler_name == 'ReduceLROnPlateau':
            scheduler.step(avg_epoch_loss_data)
        elif scheduler_name == 'cosine_schedule_with_warmup':
            pass  # cosine scheduler 在每个 batch 更新，不在这里
        else:
            scheduler.step()
        # 保存最佳模型
        if best_error > avg_epoch_loss_data:
            best_error = avg_epoch_loss_data
            model_save_record.append([e, avg_epoch_loss_data])
...
def test(config, args):
        ...
        with torch.no_grad():
            ...
            for b in tqdm(range(len(test_loader))):
                ...
                if b in [r[-1] for r in error_records_sorted[:3]]:
                    ...
                    category = 'mid'
                print('pred.squeeze().shape:', full_pred.squeeze().shape)
                # print('full_pred[:,-2:-1]:', full_pred[:, -2:-1])
                # 这里的 visualize_results 数据形状应该为[batch,nx,nt]
                visualize_results.append({
                    'Dataset': name,
                    'index': b,
                    'category': category,
                    'l2_error': l2_error,
                    'pred': full_pred.squeeze().cpu().numpy(),
                    'yy': full_yy.squeeze().cpu().numpy(),
                })
    ...
    for visualize_results in visualize_results_all:
        ...
        plot_visualization_results(visualize_results, save_dir=f'{first_dic}/figures_{case}', plot_residual=False)
    print('可视化完成~')
    # endregion
