"""
TI(L)-DeepONet for 2D Heat Equation
====================================
对标原版 2D TI-DeepONet，适配 Heat 2D 问题

与 Allen-Cahn 版的区别:
  1. 无边界条件 u_b 通道: Branch 输入 = u_window(init_step) + u_interp(1)
  2. dataloader 返回 (xx, yy, grid)，无 bc_params
  3. 物理残差使用 compute_heat2d_residual_batch
"""
...
from loss import compute_heat2d_residual_batch,compute_neumann_bc_loss
...
class BranchNet(nn.Module):
    """
    Branch Net: CNN + Pooling + Flatten + MLP
    输入通道: u_window(init_step) + u_interp(1)  (无 u_b)
    输出: [B, latent_dim]
    """
...
class LearnableRK4Net(nn.Module):
    """
    CNN + Pooling + Flatten + MLP → softmax → [B, 4]
    """
    ...
    def forward(self, u_curr):
        x = u_curr.unsqueeze(1)  # [B, 1, Nx, Ny]
        for block in self.cnn_layers:
            for layer in block:
                x = layer(x)
        x = x.flatten(start_dim=1)
...
class TIDeepONet2D(nn.Module):
    """
    TI(L)-DeepONet for 2D Heat Equation.
    Branch 输入: u_window + u_interp (无 u_b)
    """
    ...
    def __init__(self, init_step: int, latent_dim: int,
        ...
        super().__init__()
        in_channels = init_step + 1  # u_window + u_interp, 无 u_b
    ...
    def derivative_field(self, u_interp, u_window, trunk_out):
        """
        估算导数场 (无 u_b)
        ...
        Args:
            u_interp:  [B, Nx, Ny]
            u_window:  [B, T, Nx, Ny]
            trunk_out: [N_pts, latent_dim]
        Returns:
            k_field: [B, Nx, Ny]
        """
        B, Nx, Ny = u_interp.shape

        # 拼通道: [B, T+1, Nx, Ny]  (无 u_b)
        branch_in = torch.cat([
            u_window,              # [B, T, Nx, Ny]
            u_interp.unsqueeze(1), # [B, 1, Nx, Ny]
        ], dim=1)

        b_out = self.branch(branch_in)  # [B, latent_dim]
        k_flat = torch.mm(b_out, trunk_out.T) + self.bias  # [B, N_pts]
        return k_flat.reshape(B, Nx, Ny)


def rk4_step(deeponet, rk4_net, u_curr, u_window, grid_flat, dt):
    """
    一步 TI(L)-RK4 时间推进 (无 u_b)
    """
    trunk_out = deeponet.trunk(grid_flat)
    alpha = rk4_net(u_curr)

    k1 = deeponet.derivative_field(u_curr, u_window, trunk_out)
    k2 = deeponet.derivative_field(u_curr + 0.5 * dt * k1, u_window, trunk_out)
    k3 = deeponet.derivative_field(u_curr + 0.5 * dt * k2, u_window, trunk_out)
    k4 = deeponet.derivative_field(u_curr + dt * k3, u_window, trunk_out)
...
def ti_autoregressive_predict(deeponet, rk4_net, xx, grid,
    ...
    Args:
        ...
        xx:        [B, Nx, Ny, init_step, 1]
        grid:      [B, Nx, Ny, 2]
        init_t:    int
        t_end:     int
        dt:        float
    Returns:
        pred: [B, Nx, Ny, t_end, 1]
    """
    ...
    u_window = xx[..., 0].permute(0, 3, 1, 2).contiguous()  # [B, T, Nx, Ny]
    u_curr = xx[..., -1, 0]  # [B, Nx, Ny]

    for t in range(init_t, t_end):
        u_next, _ = rk4_step(deeponet, rk4_net, u_curr, u_window, grid_flat, dt)
        pred[..., t, 0] = u_next
...
def run(config, args):
    ...
    kappa = config['data'].get('kappa', 0.02)
    ...
    print(f"[TI-DeepONet Heat2D] kappa={kappa}, T={T_total}, "
    ...
    # ── 推断空间尺寸 ──────────────────────────────────────────────
    _xx, _, _ = next(iter(train_loader))
    nx, ny = _xx.shape[1], _xx.shape[2]
    in_channels = initial_step + 1  # 无 u_b
    print(f"Spatial: Nx={nx}, Ny={ny}")
    print(f"Branch CNN in_channels = {initial_step}(hist) + 1(interp) = {in_channels}")
    ...
    with open(f'{first_dic}/Experiment_record.txt', 'a', encoding='utf-8') as f:
        ...
        f.write(f"\n{'=' * 60}\n[TI(L)-DeepONet Heat2D | {ts}]\n{'=' * 60}\n")
        f.write(f"├─ Heat2D: kappa={kappa}, T={T_total}, dt={dt:.6f}\n")
    ...
    for e in ebar:
        ...
        for xx, yy, grid in tqdm(
            ...
            if warmup_epochs and e < warmup_epochs:
                ...
                if loss_mode != 'data':
                    residual = compute_heat2d_residual_batch(
                        pred, kappa=kappa, T=T_total)
                    loss_bc  = compute_neumann_bc_loss(pred)
            ...
            if warmup_epochs and e < warmup_epochs:
                ...
                if loss_mode != 'data':
                    residual_rk4 = compute_heat2d_residual_batch(
                        pred2, kappa=kappa, T=T_total)
                    loss_bc_rk4  = compute_neumann_bc_loss(pred2)
                    if residual_mode == 'mae':
...
def test(config, args):
    ...
    ckpt_path = resolve_case_path(args.pretrain) if args.pretrain else os.path.join(first_dic, 'checkpoint-best.pth.tar')
    ...
    for name, (loader, sub_x_ratio) in test_loaders.items():
        ...
        _xx_tmp, _, _ = next(iter(loader))
        cur_nx, cur_ny = _xx_tmp.shape[1], _xx_tmp.shape[2]

        if cur_nx != nx_train or cur_ny != ny_train:
            print(f"[{name}] Resolution {cur_nx}×{cur_ny} != train {nx_train}×{ny_train}, 跳过")
            continue

        with torch.no_grad():
            for b, (xx, yy, grid) in enumerate(
...
if __name__ == '__main__':
    parser = ArgumentParser(description='TI(L)-DeepONet for 2D Heat Equation')
    parser.add_argument('--config_path', type=str,
                        default='./yaml/information_don.yaml')
