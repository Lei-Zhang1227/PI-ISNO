...
def run(config, args):
    ...
    print(Nx)
    #     filepath = 'F:\data\OLdata/burgers2d_spectral.h5'
    ...
    init_t = data_config['initial_step']
    # 在循环外构造 u_b (只需一次)
    x_coord = torch.linspace(-1, 1, Nx, device=device)
    y_coord = torch.linspace(-1, 1, Nx, device=device)
    X, Y = torch.meshgrid(x_coord, y_coord, indexing='ij')
    u_b = (-0.5 * X ** 2 + 0.5 * Y ** 2).unsqueeze(0).unsqueeze(-1).unsqueeze(-1)  # [1, Nx, Ny, 1, 1]
    ...
    with torch.no_grad():
        for xx, yy, grid in test_loader:
            ...
            xx_h = xx - u_b[..., :init_t, :]
            pred_h = torch.empty(yy.shape, device=xx.device, dtype=dtype)
            pred_h[..., 0:init_t, :] = yy[..., 0:init_t, :]- u_b[..., :init_t, :]
    ...
    for name, (test_loader,sub_x) in test_loaders.items():
        ...
        with torch.no_grad():
            ...
            for b in tqdm(range(len(test_loader))):
                ...
                residual = compute_heat2d_residual_batch(yy, kappa=0.02, T=1)
            ...
            for b in tqdm(range(len(test_loader))):
                ...
                _pred = pred[..., init_t + 1:t_train, :]
                _batch = yy.size(0)
                f_p = compute_heat2d_residual_batch(pred, kappa=0.02, T=1)
                f_y = compute_heat2d_residual_batch(yy, kappa=0.02, T=1)
