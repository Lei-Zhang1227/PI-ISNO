"""
Test BC Generalization for DCT-I Model (Direct u_h Space, No BC Transform)
测试DCT-I模型在u_h空间的真正BC泛化能力

关键：
1. 提取每个测试样本自己的u_h = u_test - u_b_test
2. 模型在u_h空间预测
3. 重构 u = u_h_pred + u_b_test
4. 不做BC空间转换
"""
import os
...
def autoregressive_rollout_uh_space(
        model, xx, delta, grid, bc_params, init_t, t_train, X, Y
):
    """
    在u_h空间进行自回归rollout（不做BC转换）

    Args:
        model: DCT-I神经算子模型
        xx: [batch, Nx, Ny, init_t, 1] 初始条件（完整的u）
        yy: [batch, Nx, Ny, T, 1] ground truth（完整的u）
        grid: [batch, Nx, Ny, 2] 空间网格
        bc_params: [batch, 4] 每个样本的BC参数
        init_t: 初始步数
        t_train: 总时间步数
    ...
    # 1. 计算该样本自己的u_b
    u_b = compute_lifting(bc_params, X, Y)  # [batch, Nx, Ny, 1, 1]

    # 2. 提取u_h（输入和初始条件）
    xx_h = xx - u_b  # [batch, Nx, Ny, init_t, 1]

    # 3. 在u_h空间预测
    pred_h = torch.empty((batch, Nx, Ny, t_train, 1), device=device, dtype=dtype)
    pred_h[..., :init_t, :] = xx_h

    for t in range(init_t, t_train):
        inp = xx_h.squeeze(-1)  # [batch, Nx, Ny, init_t]
    ...
    # 4. 重构完整解 u = u_h + u_b
    pred = pred_h + u_b  # [batch, Nx, Ny, t_train, 1]

    return pred


def test_bc_generalization_uh_space(config, args):
    """
    测试DCT-I模型在u_h空间的BC泛化能力（无BC转换）
    """
    # ================================================================
    # 1. 准备工作
    # ================================================================
    ...
    print(f"\n{'=' * 80}")
    print(f"BC Generalization Test - DCT-I Model in u_h Space (No BC Transform)")
    print(f"{'=' * 80}\n")
    ...
    save_dir = os.path.join(first_dic, 'bc_generalization_test_uh_no_transform')
    ...
    print(f"\n  ⚠️  IMPORTANT: This test does NOT use BC transformation!")
    print(f"  Each sample uses its own BC to extract u_h and reconstruct u.\n")
    ...
    print(f"  Prediction mode: direct (u_h space)")
    ...
    print(
        f"  a: min={all_bc_params[:, 0].min():.3f}, max={all_bc_params[:, 0].max():.3f}, mean={all_bc_params[:, 0].mean():.3f}")
    print(
        f"  b: min={all_bc_params[:, 1].min():.3f}, max={all_bc_params[:, 1].max():.3f}, mean={all_bc_params[:, 1].mean():.3f}")
    print(
        f"  c: min={all_bc_params[:, 2].min():.3f}, max={all_bc_params[:, 2].max():.3f}, mean={all_bc_params[:, 2].mean():.3f}")
    print(
        f"  d: min={all_bc_params[:, 3].min():.3f}, max={all_bc_params[:, 3].max():.3f}, mean={all_bc_params[:, 3].mean():.3f}")
    print()
        ...
        with torch.no_grad():
            ...
            for xx, yy, grid, bc_params in tqdm(test_loader, desc=f"Testing {resolution}"):
                ...
                if first_batch:
                    ...
                    print(
                        f"  BC for first sample: [{bc_params[0, 0].item():.3f}, {bc_params[0, 1].item():.3f}, {bc_params[0, 2].item():.3f}, {bc_params[0, 3].item():.3f}]")
                    print()
                    first_batch = False

                # 关键：在u_h空间预测（不做BC转换）
                pred = autoregressive_rollout_uh_space(
                    model, xx, pre_mode, grid, bc_params,
                    init_t, t_train, X, Y
                )
                ...
                # 计算PDE残差
                residual_pred = compute_heat2d_residual_batch(pred, kappa=0.02, T=1.0)
                residual_yy = compute_heat2d_residual_batch(yy, kappa=0.02, T=1.0)
    ...
    csv_path = os.path.join(save_dir, 'bc_generalization_detailed_uh_no_transform.csv')
    ...
    summary_csv_path = os.path.join(save_dir, 'bc_generalization_summary_uh_no_transform.csv')
    summary_df.to_csv(summary_csv_path, index=False)
    print(f"Saved summary CSV to: {summary_csv_path}")

    # 保存完整结果
    pkl_path = os.path.join(save_dir, 'bc_generalization_results_uh_no_transform.pkl')
    with open(pkl_path, 'wb') as f:
        pickle.dump(all_results, f)
    print(f"Saved complete results to: {pkl_path}")

    # 保存文本报告
    report_path = os.path.join(save_dir, 'bc_generalization_report_uh_no_transform.txt')
    with open(report_path, 'w') as f:
        f.write("=" * 80 + "\n")
        f.write("BC Generalization Test Report - DCT-I Model (u_h Space, No BC Transform)\n")
        f.write("=" * 80 + "\n\n")
        ...
        f.write(f"Model: DCT-I with explicit lifting\n")
        f.write(f"Training BC (reference): {TRAIN_BC}\n")
        f.write(f"  u_b = {TRAIN_BC[0]}*x^2 + {TRAIN_BC[1]}*y^2 + {TRAIN_BC[2]}*x + {TRAIN_BC[3]}*y\n\n")

        f.write(f"⚠️  CRITICAL: This test does NOT use BC transformation!\n")
        f.write(f"   Each sample extracts u_h using its own BC: u_h = u - u_b_sample\n")
        f.write(f"   Model predicts in u_h space, then reconstructs: u = u_h + u_b_sample\n\n")

        f.write("Test BC Distribution:\n")
        f.write(
            f"  a: [{all_bc_params[:, 0].min():.3f}, {all_bc_params[:, 0].max():.3f}], mean={all_bc_params[:, 0].mean():.3f}\n")
        f.write(
            f"  b: [{all_bc_params[:, 1].min():.3f}, {all_bc_params[:, 1].max():.3f}], mean={all_bc_params[:, 1].mean():.3f}\n")
        f.write(
            f"  c: [{all_bc_params[:, 2].min():.3f}, {all_bc_params[:, 2].max():.3f}], mean={all_bc_params[:, 2].mean():.3f}\n")
        f.write(
            f"  d: [{all_bc_params[:, 3].min():.3f}, {all_bc_params[:, 3].max():.3f}], mean={all_bc_params[:, 3].mean():.3f}\n\n")
    ...
    try:
        ...
        # 保存相关性分析
        corr_csv_path = os.path.join(save_dir, 'bc_error_correlation_uh_no_transform.csv')
...
if __name__ == '__main__':
    parser = ArgumentParser(description='BC Generalization Test for DCT-I Model (u_h Space)')
    ...
    # 运行测试
    test_bc_generalization_uh_space(config, args)