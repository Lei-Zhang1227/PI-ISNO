        ...
        for b in trange(len(train_loader), file=desc, desc="batch"):
            ...
            if warmup_epochs and e < warmup_epochs:
                ...
                if config['train']['loss_mode'] != 'data':
                    residual = compute_heat2d_residual_fd(pred, kappa=0.02, T=1)
                    bc_loss = compute_neumann_bc_loss(pred)
    ...
    for name, test_loader in test_loaders.items():
        ...
        with torch.no_grad():
            ...
            for b in tqdm(range(len(test_loader))):
                ...
                residual = compute_heat2d_residual_fd(yy, kappa=0.02, T=1)
            ...
            for b in tqdm(range(len(test_loader))):
                ...
                _pred = pred[..., init_t + 1:t_train, :]
                _batch = yy.size(0)
                f_p = compute_heat2d_residual_fd(pred, kappa=0.02, T=1)
                f_y = compute_heat2d_residual_fd(yy, kappa=0.02, T=1)
