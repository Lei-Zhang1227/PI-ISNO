"""
Test BC Generalization for FFT Model WITH BC Transformation (Direct Mode)
测试FFT + Direct模式 + BC转换的BC泛化能力

关键实验：验证Direct模式下BC转换是否需要显式Lifting作为前提
"""
import os
...
def autoregressive_rollout_fft_direct_with_bc_transform(
    model, xx, yy, grid, bc_params_test, bc_params_train, init_t, t_train, X, Y
):
    """
    FFT模型的自回归rollout WITH BC TRANSFORMATION (Direct Mode)
...
def test_bc_generalization_fft_direct_with_transform(config, args):
    """
    测试FFT模型 + Direct模式 + BC转换的BC泛化能力
    """
    # ================================================================
    # 1. 准备工作
    # ================================================================
    ...
    print(f"\n{'='*80}")
    print(f"BC Generalization Test - FFT Model WITH BC Transformation (Direct Mode)")
    print(f"{'='*80}\n")
    ...
    save_dir = os.path.join(first_dic, 'bc_generalization_test_fft_direct_with_transform')
    ...
    print(f"  FFT model was trained on full u (not u_h) in DIRECT mode, but we apply BC transform at test time.\n")
    ...
    print(f"  Prediction mode: direct (with BC transform)")
    ...
    for resolution, test_loader in test_loaders.items():
        ...
        with torch.no_grad():
            ...
            for xx, yy, grid, bc_params in tqdm(test_loader, desc=f"Testing {resolution}"):
                ...
                # 关键：FFT模型 + Direct模式 + BC转换
                pred = autoregressive_rollout_fft_direct_with_bc_transform(
    ...
    csv_path = os.path.join(save_dir, 'bc_generalization_detailed_fft_direct_with_transform.csv')
    ...
    summary_csv_path = os.path.join(save_dir, 'bc_generalization_summary_fft_direct_with_transform.csv')
    summary_df.to_csv(summary_csv_path, index=False)
    print(f"Saved summary CSV to: {summary_csv_path}")
    
    # 保存完整结果
    pkl_path = os.path.join(save_dir, 'bc_generalization_results_fft_direct_with_transform.pkl')
    with open(pkl_path, 'wb') as f:
        pickle.dump(all_results, f)
    print(f"Saved complete results to: {pkl_path}")
    
    # 保存文本报告
    report_path = os.path.join(save_dir, 'bc_generalization_report_fft_direct_with_transform.txt')
    with open(report_path, 'w') as f:
        f.write("="*80 + "\n")
        f.write("BC Generalization Test Report - FFT Model WITH BC Transformation (Direct Mode)\n")
        f.write("="*80 + "\n\n")
        ...
        f.write(f"   Testing whether BC transformation works without explicit lifting (Direct mode).\n")
    ...
    try:
        ...
        # 保存相关性分析
        corr_csv_path = os.path.join(save_dir, 'bc_error_correlation_fft_direct_with_transform.csv')
...
if __name__ == '__main__':
    parser = ArgumentParser(description='BC Generalization Test for FFT Model WITH BC Transformation (Direct Mode)')
    ...
    # 运行测试
    test_bc_generalization_fft_direct_with_transform(config, args)