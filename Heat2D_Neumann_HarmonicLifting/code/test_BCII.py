...
Modified: 添加sample级别的L2误差记录（降序保存）
"""
import os
...
def test_bc_generalization(config, args):
    ...
    for resolution, test_loader in test_loaders.items():
        ...
        errors_list = []
        sample_l2_records = []  # 新增：记录每个sample的L2和key
        
        with torch.no_grad():
            first_batch = True
            sample_idx = 0  # 用于索引test_keys
            for xx, yy, grid, bc_params in tqdm(test_loader, desc=f"Testing {resolution}"):
                ...
                # 新增：记录sample级别的L2误差和key
                sample_key = test_keys[sample_idx] if sample_idx < len(test_keys) else f"sample_{sample_idx}"
                sample_l2_records.append({
        ...
        all_results[resolution] = {
            ...
            'bc_params': bc_array,
            'sample_l2_records': sample_l2_records,  # 新增
        }
    ...
    # 新增：保存sample级别的L2误差（降序排列）
    for resolution, results in all_results.items():
        sample_l2_records = results['sample_l2_records']
        
        # 转换为DataFrame并按L2误差降序排列
        sample_df = pd.DataFrame(sample_l2_records)
        sample_df_sorted = sample_df.sort_values('l2_error', ascending=False).reset_index(drop=True)
        
        # 添加排名列
        sample_df_sorted.insert(0, 'rank', range(1, len(sample_df_sorted) + 1))
        
        # 保存CSV
        sample_csv_path = os.path.join(save_dir, f'sample_l2_errors_{resolution}_sorted.csv')
        sample_df_sorted.to_csv(sample_csv_path, index=False)
        print(f"Saved sample L2 errors ({resolution}, sorted descending) to: {sample_csv_path}")
        
        # 保存numpy数组
        sample_array = np.array([
            [rec['sample_key'], rec['l2_error']] 
            for rec in sample_l2_records
        ])
        # 按L2误差降序排列
        sorted_indices = np.argsort([rec['l2_error'] for rec in sample_l2_records])[::-1]
        sample_array_sorted = sample_array[sorted_indices]
        
        sample_npy_path = os.path.join(save_dir, f'sample_l2_errors_{resolution}_sorted.npy')
        np.save(sample_npy_path, sample_array_sorted)
        print(f"Saved sample L2 array ({resolution}, sorted descending) to: {sample_npy_path}")
