"""
Allen-Cahn 2D Extended Testing Script - FIXED VERSION
修正了覆盖逻辑的bug：只覆盖 t < init_t 的部分，而不是 t < initial_step
"""
import os
import sys
import pandas as pd
...
def testII(config, args):
    """
    Allen-Cahn方程的全面评测函数
    - 计算seen/unseen数据集的L2误差
    - 分段统计误差（0.1-1, 1-1.5, ..., 4.5-5）
    - 逐时间步误差统计
    - 样本选择策略：按训练阶段误差选择8个 + 按整体误差选择2个
    """
    # region prepare
    ################################################################
    # prepare
    ...
    initial_step = 5
    filepath = case_path(config['data'].get('extend_datapath', 'data/ac2d_extend_200.h5'))
    ...
    # AC: input = u_window(init_step) + grid(2) + u_b_field(1)
    ...
    print(f"  input_channel={input_channel} (u:{initial_step} + grid:2 + u_b:1)")
    # endregion
    ...
    for name, test_loader in test_loaders.items():
        ...
        with torch.no_grad():
            ...
            for b in tqdm(range(len(test_loader)), desc=name):
                ...
                # ⚠️ 修正：只覆盖 t < init_t 的部分（t=0,1,2）
                # 而不是 t < initial_step（这会覆盖 t=0,1,2,3,4，破坏预测结果）
                pred[..., :init_t, :] = yy[..., :init_t, :]
                ...
                sample_details.append({
                    ...
                    'bc_params': bc_params.squeeze().cpu().numpy(),  # AC特有：保存BC参数
                })
...
if __name__ == '__main__':
    parser = ArgumentParser(description='Allen-Cahn 2D Extended Testing')
