...
if __name__ == '__main__':
    if __name__ == '__main__':
        ...
        model = DeepONet2D(in_channels=init_t + 2, out_channels=out_ch).to(device)
        model.eval()

        # 虚拟输入: [batch, Nx, Ny, init_t + 2]
        x = torch.randn(batch, Nx, Ny, init_t + 2).to(device)
        ...
        # 自回归模拟
        print("\n--- 自回归测试 (3步) ---")
        xx = torch.randn(batch, Nx, Ny, init_t, 1).to(device)
        grid = torch.randn(batch, Nx, Ny, 2).to(device)
        ...
        for t in range(3):
            ...
            last_step = xx[..., -1:, :]
            out = last_step + out
            print(f"  Step {t}: inp {list(inp.shape)} -> out {list(out.shape)}")
