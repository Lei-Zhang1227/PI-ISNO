...
class FNODatasetMult(Dataset):
    def __init__(self, filename,
        ...
        self.file_path = case_path('data/2D_diff-react_NA_NA.h5')
...
def run(config):
    ...
    file_path = case_path('data/2D_diff-react_4_1.h5')
    ...
    train_data = FNODatasetMult(filename=case_path(data_config['datapath']),
                                initial_step=initial_step,
                                reduced_resolution=1,
                                reduced_resolution_t=1,
                                reduced_batch=10)
    val_data = FNODatasetMult(filename=case_path(data_config['datapath']),
                              initial_step=initial_step,
                              reduced_resolution=1,
                              reduced_resolution_t=1,
                              reduced_batch=10,
    ...
    train_loader = torch.utils.data.DataLoader(train_data, batch_size=10, num_workers=4, shuffle=True)
    val_loader = torch.utils.data.DataLoader(val_data, batch_size=10, num_workers=4, shuffle=False)
    # train_data = FNODatasetMultII(filename=file_path,
    #                               initial_step=initial_step, if_test=False)
    # val_data = FNODatasetMultII(filename=file_path,
    #                             initial_step=initial_step, if_test=True)
    ...
    writer = SummaryWriter(log_dir=os.path.join(first_dic, "logs"))  #
    # endregion
    # region model
    ################################################################
    # model
    ...
    for e in ebar:
        ...
        for b in trange(len(train_loader), file=desc, desc="batch"):
            ...
            for t in range(initial_step, t_train):
                ...
                y = yy[..., t:t + 1, :]
                delta = model(torch.cat([inp, grid], dim=-1)).reshape(outp_shape)
                out = xx[..., -1:, :] + delta
                _batch = out.size(0)
...
def test(config, args):
    ...
    batch_size = config['train']['batchsize']
    initial_step = config['train']['initial_step']
    TEST_1 = FNODatasetMult(filename=case_path(data_config['datapath']),
                            initial_step=initial_step,
                            reduced_resolution=1,
                            reduced_resolution_t=1,
                            reduced_batch=5,
    ...
    TEST_2 = FNODatasetMultII(filename=case_path('data/2D_diff-react_2_1.h5'),
                              initial_step=initial_step, if_test=True)
    loader_2 = torch.utils.data.DataLoader(TEST_2, batch_size=1, num_workers=28, shuffle=False)

    TEST_3 = FNODatasetMultII(filename=case_path('data/2D_diff-react_4_1.h5'),
                              initial_step=initial_step, if_test=True)
